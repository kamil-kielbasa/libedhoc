...
#ifdef __ZEPHYR__
K_MUTEX_DEFINE(edhoc_cipher_suite_24_mutex);
#else /* __ZEPHYR__ */
static pthread_mutex_t edhoc_cipher_suite_24_mutex = PTHREAD_MUTEX_INITIALIZER;
#endif /* __ZEPHYR__ */
		...
		size_t sign_size, size_t *sign_len)
{
	...
	if (sign_size < EDHOC_CIPHER_SUITE_24_ECC_ECDSA_SIGN_LEN) {
		  ...
		  const uint8_t *sign, size_t sign_len)
{
	...
	if (EDHOC_CIPHER_SUITE_24_ECC_UNCOMP_KEY_LEN != pub_key_len) {
...
{
#ifdef __ZEPHYR__
	return k_mutex_lock(&edhoc_cipher_suite_24_mutex, K_FOREVER);
#else /* __ZEPHYR__ */
	return pthread_mutex_lock(&edhoc_cipher_suite_24_mutex);
#endif /* __ZEPHYR__ */
}
...
{
#ifdef __ZEPHYR__
	return k_mutex_unlock(&edhoc_cipher_suite_24_mutex);
#else /* __ZEPHYR__ */
	return pthread_mutex_unlock(&edhoc_cipher_suite_24_mutex);
#endif /* __ZEPHYR__ */
}
