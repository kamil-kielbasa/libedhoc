...
#define EDHOC_CIPHER_SUITE_0_ECC_KEY_BITS (255)

#define EDHOC_CIPHER_SUITE_0_EDDSA_SIGN_LEN (64)

#define EDHOC_CIPHER_SUITE_0_HASH_LEN (32)
#define EDHOC_CIPHER_SUITE_0_HASH_ALG (PSA_ALG_SHA_256)
...
#define EDHOC_CIPHER_SUITE_0_KDF_HASH_ALG (PSA_ALG_SHA_256)

#define EDHOC_CIPHER_SUITE_0_HASH_OP_POOL_SIZE (4)
...
static bool hash_op_in_use[EDHOC_CIPHER_SUITE_0_HASH_OP_POOL_SIZE];
...
static void set_derive_key_attributes(psa_key_attributes_t *attr)
{
	...
	psa_set_key_algorithm(
		attr, PSA_ALG_HKDF_EXPAND(EDHOC_CIPHER_SUITE_0_KDF_HASH_ALG));
	psa_set_key_enrollment_algorithm(
		attr, PSA_ALG_HKDF_EXTRACT(EDHOC_CIPHER_SUITE_0_KDF_HASH_ALG));
}
...
static void set_x25519_keypair_attributes(psa_key_attributes_t *attr)
{
	...
	psa_set_key_bits(attr, EDHOC_CIPHER_SUITE_0_ECC_KEY_BITS);
}
			     ...
			     size_t *out_len)
{
	...
	if (out_size < EDHOC_CIPHER_SUITE_0_ECC_KEY_LEN) {
	...
	if (PSA_SUCCESS != status ||
	    EDHOC_CIPHER_SUITE_0_ECC_KEY_LEN != exported_len) {
		EDHOC_LOG_ERR("Export public key: %d", status);
		return EDHOC_ERROR_CRYPTO_FAILURE;
	}

	*out_len = EDHOC_CIPHER_SUITE_0_ECC_KEY_LEN;
		...
		size_t sign_size, size_t *sign_len)
{
	...
	if (EDHOC_CIPHER_SUITE_0_EDDSA_SIGN_LEN != sign_size) {
		  ...
		  const uint8_t *sign, size_t sign_len)
{
	...
	if (EDHOC_CIPHER_SUITE_0_EDDSA_SIGN_LEN != sign_len) {
		   ...
		   const uint8_t *salt, size_t salt_len, void *prk_key_id)
{
	...
	psa_status_t status = psa_key_derivation_setup(
		&op, PSA_ALG_HKDF_EXTRACT(EDHOC_CIPHER_SUITE_0_KDF_HASH_ALG));
		  ...
		  enum edhoc_key_usage usage, void *out_key_id)
{
	...
	case EDHOC_KEY_USAGE_KDF:
		set_derive_key_attributes(&attr);
		output_length = EDHOC_CIPHER_SUITE_0_HASH_LEN;
		break;
	case EDHOC_KEY_USAGE_AEAD:
		...
		output_length = EDHOC_CIPHER_SUITE_0_AEAD_KEY_LEN;
		break;
	default:
	...
	psa_status_t status = psa_key_derivation_setup(
		&op, PSA_ALG_HKDF_EXPAND(EDHOC_CIPHER_SUITE_0_KDF_HASH_ALG));
	...
	*op = NULL;
	...
	for (size_t i = 0; i < ARRAY_SIZE(hash_op_pool); ++i) {
		...
		const psa_status_t status = psa_hash_setup(
			&hash_op_pool[i], EDHOC_CIPHER_SUITE_0_HASH_ALG);
