...
#define EDHOC_CIPHER_SUITE_2_ECC_ECDH_SHARED_SECRET_LEN (32)

#define EDHOC_CIPHER_SUITE_2_HASH_LEN (32)
#define EDHOC_CIPHER_SUITE_2_HASH_ALG (PSA_ALG_SHA_256)
...
#define EDHOC_CIPHER_SUITE_2_KDF_HASH_ALG (PSA_ALG_SHA_256)

#define EDHOC_CIPHER_SUITE_2_HASH_OP_POOL_SIZE (4)
...
static psa_hash_operation_t hash_op_pool[EDHOC_CIPHER_SUITE_2_HASH_OP_POOL_SIZE];
static bool hash_op_in_use[EDHOC_CIPHER_SUITE_2_HASH_OP_POOL_SIZE];
...
/** \brief Set attributes for an ephemeral P-256 ECDH key pair. */
static void set_ecdh_keypair_attributes(psa_key_attributes_t *attr);

/** \brief Export a P-256 public key as its 32-byte X coordinate (G_X / G_Y). */
...
/** \brief Generate an ES256 signature (\ref edhoc_crypto.sign). */
...
/** \brief Verify an ES256 signature (\ref edhoc_crypto.verify). */
...
static void set_derive_key_attributes(psa_key_attributes_t *attr)
{
	...
	psa_set_key_algorithm(
		attr, PSA_ALG_HKDF_EXPAND(EDHOC_CIPHER_SUITE_2_KDF_HASH_ALG));
	psa_set_key_enrollment_algorithm(
		attr, PSA_ALG_HKDF_EXTRACT(EDHOC_CIPHER_SUITE_2_KDF_HASH_ALG));
}
			       ...
			       size_t *out_len)
{
	...
	if (out_size < EDHOC_CIPHER_SUITE_2_ECC_COMP_KEY_LEN) {
		EDHOC_LOG_ERR("Public key buffer too small: %zu", out_size);
		return EDHOC_ERROR_BUFFER_TOO_SMALL;
	}

	uint8_t uncomp[EDHOC_CIPHER_SUITE_2_ECC_UNCOMP_KEY_LEN] = { 0 };
	size_t uncomp_len = 0;

	const psa_status_t status =
		psa_export_public_key(key, uncomp, sizeof(uncomp), &uncomp_len);

	if (PSA_SUCCESS != status ||
	    EDHOC_CIPHER_SUITE_2_ECC_UNCOMP_KEY_LEN != uncomp_len) {
		EDHOC_LOG_ERR("Export public key: %d", status);
		return EDHOC_ERROR_CRYPTO_FAILURE;
	}

	/* Uncompressed point is 0x04 | X | Y; EDHOC transports X only. */
	(void)memcpy(out, &uncomp[1], EDHOC_CIPHER_SUITE_2_ECC_COMP_KEY_LEN);
	*out_len = EDHOC_CIPHER_SUITE_2_ECC_COMP_KEY_LEN;
				 ...
				 psa_key_id_t *shr_sec_key)
{
	...
	EDHOC_ASSERT(EDHOC_CIPHER_SUITE_2_ECC_COMP_KEY_LEN == peer_pub_key_len);
	...
	*shr_sec_key = PSA_KEY_ID_NULL;
	...
	uint8_t decomp_key[EDHOC_CIPHER_SUITE_2_ECC_UNCOMP_KEY_LEN] = { 0 };
	...
	psa_set_key_bits(
		...
		(size_t)PSA_BYTES_TO_BITS(
			EDHOC_CIPHER_SUITE_2_ECC_ECDH_SHARED_SECRET_LEN));
			     ...
			     size_t *encaps_key_len)
{
	...
	*psa_kid = PSA_KEY_ID_NULL;
	...
	const int ret = export_public_key_x(*psa_kid, encaps_key,
		       ...
		       size_t ciphertext_size, size_t *ciphertext_len)
{
	...
	*shared_secret_kid = PSA_KEY_ID_NULL;
	...
	int ret = export_public_key_x(ephemeral_kid, ciphertext,
		...
		size_t sign_size, size_t *sign_len)
{
	...
	psa_status_t status = psa_hash_compute(EDHOC_CIPHER_SUITE_2_HASH_ALG,
					       input, input_len, digest,
					       sizeof(digest), &digest_len);

	if (PSA_SUCCESS != status ||
	    PSA_HASH_LENGTH(EDHOC_CIPHER_SUITE_2_HASH_ALG) != digest_len) {
		EDHOC_LOG_ERR("Hash input: %d", status);
		return EDHOC_ERROR_CRYPTO_FAILURE;
	}

	status = psa_sign_hash(private_kid,
			       PSA_ALG_ECDSA(EDHOC_CIPHER_SUITE_2_HASH_ALG),
		  ...
		  const uint8_t *sign, size_t sign_len)
{
	...
	psa_key_id_t public_kid = PSA_KEY_ID_NULL;
	status = psa_import_key(&attr, pub_key, pub_key_len, &public_kid);
	...
	status = psa_verify_hash(public_kid,
				 PSA_ALG_ECDSA(EDHOC_CIPHER_SUITE_2_HASH_ALG),
				 digest, digest_len, sign, sign_len);

	destroy_volatile_key(public_kid);
		   ...
		   const uint8_t *salt, size_t salt_len, void *prk_key_id)
{
	...
	*prk_kid = PSA_KEY_ID_NULL;
	...
	psa_status_t status = psa_key_derivation_setup(
		&op, PSA_ALG_HKDF_EXTRACT(EDHOC_CIPHER_SUITE_2_KDF_HASH_ALG));
		  ...
		  enum edhoc_key_usage usage, void *out_key_id)
{
	...
	*output_kid = PSA_KEY_ID_NULL;
	...
	case EDHOC_KEY_USAGE_KDF:
		set_derive_key_attributes(&attr);
		output_length = EDHOC_CIPHER_SUITE_2_HASH_LEN;
		break;
	case EDHOC_KEY_USAGE_AEAD:
		...
		output_length = EDHOC_CIPHER_SUITE_2_AEAD_KEY_LEN;
		break;
	default:
	...
	psa_status_t status = psa_key_derivation_setup(
		&op, PSA_ALG_HKDF_EXPAND(EDHOC_CIPHER_SUITE_2_KDF_HASH_ALG));
	...
	*op = NULL;
	...
	for (size_t i = 0; i < ARRAY_SIZE(hash_op_pool); ++i) {
		...
		const psa_status_t status = psa_hash_setup(
			&hash_op_pool[i], EDHOC_CIPHER_SUITE_2_HASH_ALG);
...
const struct edhoc_cipher_suite *edhoc_cipher_suite_2_get_suite(void)
{
	static const struct edhoc_cipher_suite suite = {
		...
		.kem_public_key_length = EDHOC_CIPHER_SUITE_2_ECC_COMP_KEY_LEN,
		.kem_ciphertext_length = EDHOC_CIPHER_SUITE_2_ECC_COMP_KEY_LEN,
		.nike_key_length = EDHOC_CIPHER_SUITE_2_ECC_COMP_KEY_LEN,
		.sign_length = EDHOC_CIPHER_SUITE_2_ECC_ECDSA_SIGN_LEN,
