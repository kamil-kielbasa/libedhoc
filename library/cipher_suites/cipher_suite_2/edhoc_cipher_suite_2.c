...
#ifdef __ZEPHYR__
K_MUTEX_DEFINE(edhoc_cipher_suite_2_mutex);
#else /* __ZEPHYR__ */
static pthread_mutex_t edhoc_cipher_suite_2_mutex = PTHREAD_MUTEX_INITIALIZER;
#endif /* __ZEPHYR__ */
				  ...
				  size_t *decomp_key_len)
{
	...
	/* LCOV_EXCL_START */
	if (NULL == grp || NULL == raw_key || 0 == raw_key_len ||
	    NULL == decomp_key || 0 == decomp_key_size ||
	    NULL == decomp_key_len) {
...
{
#ifdef __ZEPHYR__
	return k_mutex_lock(&edhoc_cipher_suite_2_mutex, K_FOREVER);
#else /* __ZEPHYR__ */
	return pthread_mutex_lock(&edhoc_cipher_suite_2_mutex);
#endif /* __ZEPHYR__ */
}
...
{
#ifdef __ZEPHYR__
	return k_mutex_unlock(&edhoc_cipher_suite_2_mutex);
#else /* __ZEPHYR__ */
	return pthread_mutex_unlock(&edhoc_cipher_suite_2_mutex);
#endif /* __ZEPHYR__ */
}
