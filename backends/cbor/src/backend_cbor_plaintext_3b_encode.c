...
static bool encode_plaintext_3b(zcbor_state_t *state, const struct plaintext_3b *input);
...
static bool encode_plaintext_3b(
		zcbor_state_t *state, const struct plaintext_3b *input)
{
	zcbor_log("%s\r\n", __func__);

	bool tmp_result = ((!(*input).plaintext_3b_present || encode_EAD_3(state, (&(*input).plaintext_3b))));
...
int cbor_encode_plaintext_3b(
		uint8_t *payload, size_t payload_len,
		const struct plaintext_3b *input,
		size_t *payload_len_out)
{
	zcbor_state_t states[2];

	return zcbor_entry_function(payload, payload_len, (void *)input, payload_len_out, states,
		(zcbor_decoder_t *)encode_plaintext_3b, sizeof(states) / sizeof(zcbor_state_t), 6);
}
