		...
		zcbor_state_t *state, struct sig_structure *result)
{
	...
	&& ((zcbor_bstr_decode(state, (&(*result)._sig_structure_protected))))
	&& ((zcbor_bstr_decode(state, (&(*result)._sig_structure_external_aad))))
	&& ((zcbor_bstr_decode(state, (&(*result)._sig_structure_payload))))) || (zcbor_list_map_end_force_decode(state), false)) && zcbor_list_end_decode(state))));
