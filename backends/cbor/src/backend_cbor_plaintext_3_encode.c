		...
		zcbor_state_t *state, const struct plaintext_3 *input)
{
	...
	: (((*input).plaintext_3_ID_CRED_I_choice == plaintext_3_ID_CRED_I_id_cred_x_m_c) ? ((encode_id_cred_x(state, (&(*input).plaintext_3_ID_CRED_I_id_cred_x_m))))
	: false))))
	&& ((zcbor_bstr_encode(state, (&(*input).plaintext_3_Signature_or_MAC_3))))
	&& (!(*input).plaintext_3_ead_m_present || encode_ead(state, (&(*input).plaintext_3_ead_m))))));
