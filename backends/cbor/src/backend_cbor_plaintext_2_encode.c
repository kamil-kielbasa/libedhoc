		...
		zcbor_state_t *state, const struct plaintext_2 *input)
{
	...
	: (((*input).plaintext_2_ID_CRED_R_choice == plaintext_2_ID_CRED_R_id_cred_x_m_c) ? ((encode_id_cred_x(state, (&(*input).plaintext_2_ID_CRED_R_id_cred_x_m))))
	: false))))
	&& ((zcbor_bstr_encode(state, (&(*input).plaintext_2_Signature_or_MAC_2))))
	&& (!(*input).plaintext_2_ead_m_present || encode_ead(state, (&(*input).plaintext_2_ead_m))))));
