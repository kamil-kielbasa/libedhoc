		...
		zcbor_state_t *state, struct plaintext_3 *result)
{
	...
	bool tmp_result = (((((zcbor_union_start_code(state) && (int_res = ((((zcbor_int32_decode(state, (&(*result).plaintext_3_ID_CRED_I_int)))) && (((*result).plaintext_3_ID_CRED_I_choice = plaintext_3_ID_CRED_I_int_c), true))
	|| (((zcbor_bstr_decode(state, (&(*result).plaintext_3_ID_CRED_I_bstr)))) && (((*result).plaintext_3_ID_CRED_I_choice = plaintext_3_ID_CRED_I_bstr_c), true))
	|| (zcbor_union_elem_code(state) && (((decode_id_cred_x(state, (&(*result).plaintext_3_ID_CRED_I_id_cred_x_m)))) && (((*result).plaintext_3_ID_CRED_I_choice = plaintext_3_ID_CRED_I_id_cred_x_m_c), true)))), zcbor_union_end_code(state), int_res)))
	&& ((zcbor_bstr_decode(state, (&(*result).plaintext_3_Signature_or_MAC_3))))
	&& ((*result).plaintext_3_ead_m_present = ((decode_ead(state, (&(*result).plaintext_3_ead_m)))), 1))));
		...
		size_t *payload_len_out)
{
	...
	return zcbor_entry_function(payload, payload_len, (void *)result, payload_len_out, states,
		(zcbor_decoder_t *)decode_plaintext_3, sizeof(states) / sizeof(zcbor_state_t), 8);
}
