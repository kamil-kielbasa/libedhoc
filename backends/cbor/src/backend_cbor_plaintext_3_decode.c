		...
		zcbor_state_t *state, struct plaintext_3 *result)
{
	...
	|| (zcbor_union_elem_code(state) && (((decode_map(state, (&(*result).plaintext_3_ID_CRED_I_map_m)))) && (((*result).plaintext_3_ID_CRED_I_choice = plaintext_3_ID_CRED_I_map_m_c), true)))), zcbor_union_end_code(state), int_res)))
	&& ((zcbor_bstr_decode(state, (&(*result).plaintext_3_Signature_or_MAC_3))))
	&& ((*result).plaintext_3_EAD_3_m_present = ((decode_EAD_3(state, (&(*result).plaintext_3_EAD_3_m)))), 1))));
