...
static bool decode_plaintext_3a(zcbor_state_t *state, struct plaintext_3a *result);
...
static bool decode_plaintext_3a(
		zcbor_state_t *state, struct plaintext_3a *result)
{
	zcbor_log("%s\r\n", __func__);
	bool int_res;

	bool tmp_result = (((((zcbor_union_start_code(state) && (int_res = ((((decode_header_map(state, (&(*result).plaintext_3a_ID_CRED_PSK_header_map_m)))) && (((*result).plaintext_3a_ID_CRED_PSK_choice = plaintext_3a_ID_CRED_PSK_header_map_m_c), true))
	|| (zcbor_union_elem_code(state) && (((zcbor_bstr_decode(state, (&(*result).plaintext_3a_ID_CRED_PSK_bstr)))) && (((*result).plaintext_3a_ID_CRED_PSK_choice = plaintext_3a_ID_CRED_PSK_bstr_c), true)))
	|| (((zcbor_int32_decode(state, (&(*result).plaintext_3a_ID_CRED_PSK_int)))
	&& ((((*result).plaintext_3a_ID_CRED_PSK_int >= -24)
	&& ((*result).plaintext_3a_ID_CRED_PSK_int <= 23)) || (zcbor_error(state, ZCBOR_ERR_WRONG_RANGE), false))) && (((*result).plaintext_3a_ID_CRED_PSK_choice = plaintext_3a_ID_CRED_PSK_int_c), true))), zcbor_union_end_code(state), int_res)))
	&& ((zcbor_bstr_decode(state, (&(*result).plaintext_3a_CIPHERTEXT_3B)))))));
...
int cbor_decode_plaintext_3a(
		const uint8_t *payload, size_t payload_len,
		struct plaintext_3a *result,
		size_t *payload_len_out)
{
	zcbor_state_t states[6];

	return zcbor_entry_function(payload, payload_len, (void *)result, payload_len_out, states,
		(zcbor_decoder_t *)decode_plaintext_3a, sizeof(states) / sizeof(zcbor_state_t), 2);
}
