...
libedhoc_options+="-DLIBEDHOC_ENABLE_MODULE_TESTS_TRACES=OFF "
libedhoc_options+="-DLIBEDHOC_BUILD_COMPILER_GCC=ON "
...
kconfig_options+="-DCONFIG_LIBEDHOC_MAX_LEN_OF_CONN_ID=1 "
...
mkdir build
cd build
cmake $libedhoc_options $mbedtls_options $kconfig_options -DCMAKE_BUILD_TYPE=Debug ..
make -j
