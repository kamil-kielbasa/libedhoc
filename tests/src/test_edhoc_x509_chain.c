...
static const struct edhoc_crypto crypto_init_mocked = {
	.make_key_pair = test_crypto_make_key_pair_init_mocked_x509_chain,
	.key_agreement = test_crypto_key_agreement,
	.sign = test_crypto_sign_init_mocked_x509_chain,
...
static const struct edhoc_credentials cred_init_mocked = {
	...
	.verify = test_cred_verify_init_mocked_x509_chain,
};
...
void test_edhoc_x509_chain_message_1_compose(void)
{
	...
	/**
         ...
         */
	size_t msg_1_len = 0;
	uint8_t msg_1[ARRAY_SIZE(test_vector_1_message_1)] = { 0 };
	ret = edhoc_message_1_compose(&init_ctx, msg_1, ARRAY_SIZE(msg_1),
				      &msg_1_len);
	assert(EDHOC_SUCCESS == ret);
	assert(WAIT_M2 == init_ctx.status);

	assert(ARRAY_SIZE(test_vector_1_message_1) == msg_1_len);
	assert(0 == memcmp(test_vector_1_message_1, msg_1, msg_1_len));

	assert(EDHOC_PRK_STATE_INVALID == init_ctx.prk_state);
	assert(0 == init_ctx.prk_len);

	assert(EDHOC_TH_STATE_1 == init_ctx.th_state);
	assert(ARRAY_SIZE(test_vector_1_h_message_1_raw) == init_ctx.th_len);
	assert(0 == memcmp(test_vector_1_h_message_1_raw, init_ctx.th,
			   init_ctx.th_len));

	assert(ARRAY_SIZE(test_vector_1_x_raw) == init_ctx.dh_priv_key_len);
	assert(0 == memcmp(test_vector_1_x_raw, init_ctx.dh_priv_key,
...
void test_edhoc_x509_chain_message_1_process(void)
{
	...
	/**
         ...
         */
	ret = edhoc_message_1_process(&resp_ctx, test_vector_1_message_1,
				      ARRAY_SIZE(test_vector_1_message_1));
	assert(EDHOC_SUCCESS == ret);
	assert(VERIFIED_M1 == resp_ctx.status);

	assert(EDHOC_TH_STATE_1 == resp_ctx.th_state);
	assert(ARRAY_SIZE(test_vector_1_h_message_1_raw) == resp_ctx.th_len);
	assert(0 == memcmp(test_vector_1_h_message_1_raw, resp_ctx.th,
			   resp_ctx.th_len));
	...
	assert((int8_t)test_vector_1_c_i_raw == (int8_t)resp_ctx.peer_cid[0]);

	assert(ARRAY_SIZE(test_vector_1_g_x_raw) ==
	       resp_ctx.dh_peer_pub_key_len);
	assert(0 == memcmp(test_vector_1_g_x_raw, resp_ctx.dh_peer_pub_key,
			   resp_ctx.dh_peer_pub_key_len));
...
void test_edhoc_x509_chain_message_2_compose(void)
{
	...
	resp_ctx.th_len = ARRAY_SIZE(test_vector_1_h_message_1_raw);
	memcpy(resp_ctx.th, test_vector_1_h_message_1_raw,
	       sizeof(test_vector_1_h_message_1_raw));

	resp_ctx.peer_cid[0] = (int8_t)test_vector_1_c_i_raw;
	resp_ctx.peer_cid_len = 1;

	resp_ctx.dh_peer_pub_key_len = ARRAY_SIZE(test_vector_1_g_x_raw);
	memcpy(resp_ctx.dh_peer_pub_key, test_vector_1_g_x_raw,
	...
	/**
         ...
         */
	size_t msg_2_len = 0;
	uint8_t msg_2[ARRAY_SIZE(test_vector_1_message_2)] = { 0 };
	ret = edhoc_message_2_compose(&resp_ctx, msg_2, ARRAY_SIZE(msg_2),
				      &msg_2_len);
	assert(EDHOC_SUCCESS == ret);
	assert(WAIT_M3 == resp_ctx.status);

	assert(ARRAY_SIZE(test_vector_1_message_2) == msg_2_len);
	assert(0 == memcmp(test_vector_1_message_2, msg_2, msg_2_len));

	assert(ARRAY_SIZE(test_vector_1_g_xy_raw) == resp_ctx.dh_secret_len);
	assert(0 == memcmp(test_vector_1_g_xy_raw, resp_ctx.dh_secret,
			   sizeof(resp_ctx.dh_secret)));

	assert(EDHOC_TH_STATE_3 == resp_ctx.th_state);
	assert(ARRAY_SIZE(test_vector_1_th_3_raw) == resp_ctx.th_len);
	assert(0 ==
	       memcmp(test_vector_1_th_3_raw, resp_ctx.th, resp_ctx.th_len));

	assert(EDHOC_PRK_STATE_3E2M == resp_ctx.prk_state);
	assert(ARRAY_SIZE(test_vector_1_prk_3e2m_raw) == resp_ctx.prk_len);
	assert(0 == memcmp(test_vector_1_prk_3e2m_raw, resp_ctx.prk,
			   resp_ctx.prk_len));
...
void test_edhoc_x509_chain_message_2_process(void)
{
	...
	init_ctx.dh_priv_key_len = ARRAY_SIZE(test_vector_1_x_raw);
	memcpy(init_ctx.dh_priv_key, test_vector_1_x_raw,
	       init_ctx.dh_priv_key_len);

	init_ctx.th_state = EDHOC_TH_STATE_1;
	init_ctx.th_len = ARRAY_SIZE(test_vector_1_h_message_1_raw);
	memcpy(init_ctx.th, test_vector_1_h_message_1_raw, init_ctx.th_len);

	/**
         * \brief EDHOC message 2 process.
         */
	ret = edhoc_message_2_process(&init_ctx, test_vector_1_message_2,
				      ARRAY_SIZE(test_vector_1_message_2));

	assert(EDHOC_SUCCESS == ret);
	assert(VERIFIED_M2 == init_ctx.status);

	assert(1 == init_ctx.peer_cid_len);
	assert(0 == memcmp(init_ctx.peer_cid, &test_vector_1_c_r_raw,
			   init_ctx.peer_cid_len));

	assert(ARRAY_SIZE(test_vector_1_g_xy_raw) == init_ctx.dh_secret_len);
	assert(0 == memcmp(init_ctx.dh_secret, test_vector_1_g_xy_raw,
			   sizeof(init_ctx.dh_secret)));

	assert(EDHOC_TH_STATE_3 == init_ctx.th_state);
	assert(ARRAY_SIZE(test_vector_1_th_3_raw) == init_ctx.th_len);
	assert(0 ==
	       memcmp(init_ctx.th, test_vector_1_th_3_raw, init_ctx.th_len));

	assert(EDHOC_PRK_STATE_3E2M == init_ctx.prk_state);
	assert(ARRAY_SIZE(test_vector_1_prk_3e2m_raw) == init_ctx.prk_len);
	assert(0 == memcmp(test_vector_1_prk_3e2m_raw, init_ctx.prk,
			   init_ctx.prk_len));
...
void test_edhoc_x509_chain_message_3_compose(void)
{
	...
	init_ctx.dh_secret_len = ARRAY_SIZE(test_vector_1_g_xy_raw);
	memcpy(init_ctx.dh_secret, test_vector_1_g_xy_raw,
	       sizeof(init_ctx.dh_secret));

	init_ctx.th_state = EDHOC_TH_STATE_3;
	init_ctx.th_len = ARRAY_SIZE(test_vector_1_th_3_raw);
	memcpy(init_ctx.th, test_vector_1_th_3_raw, init_ctx.th_len);

	init_ctx.prk_state = EDHOC_PRK_STATE_3E2M;
	init_ctx.prk_len = ARRAY_SIZE(test_vector_1_prk_3e2m_raw);
	memcpy(init_ctx.prk, test_vector_1_prk_3e2m_raw, init_ctx.prk_len);

	/**
         * \brief EDHOC message 3 compose.
         */
	size_t msg_3_len = 0;
	uint8_t msg_3[ARRAY_SIZE(test_vector_1_message_3)] = { 0 };
	...
	assert(ARRAY_SIZE(test_vector_1_message_3) == msg_3_len);
	assert(0 == memcmp(test_vector_1_message_3, msg_3, msg_3_len));

	assert(EDHOC_TH_STATE_4 == init_ctx.th_state);
	assert(ARRAY_SIZE(test_vector_1_th_4_raw) == init_ctx.th_len);
	assert(0 ==
	       memcmp(test_vector_1_th_4_raw, init_ctx.th, init_ctx.th_len));

	assert(EDHOC_PRK_STATE_OUT == init_ctx.prk_state);
	assert(ARRAY_SIZE(test_vector_1_prk_out) == init_ctx.prk_len);
	assert(0 ==
	       memcmp(test_vector_1_prk_out, init_ctx.prk, init_ctx.prk_len));
...
void test_edhoc_x509_chain_message_3_process(void)
{
	...
	resp_ctx.dh_secret_len = ARRAY_SIZE(test_vector_1_g_xy_raw);
	memcpy(resp_ctx.dh_secret, test_vector_1_g_xy_raw,
	       resp_ctx.dh_secret_len);

	resp_ctx.th_state = EDHOC_TH_STATE_3;
	resp_ctx.th_len = ARRAY_SIZE(test_vector_1_th_3_raw);
	memcpy(resp_ctx.th, test_vector_1_th_3_raw, resp_ctx.th_len);

	resp_ctx.prk_state = EDHOC_PRK_STATE_3E2M;
	resp_ctx.prk_len = ARRAY_SIZE(test_vector_1_prk_3e2m_raw);
	memcpy(resp_ctx.prk, test_vector_1_prk_3e2m_raw, resp_ctx.prk_len);

	/**
         * \brief EDHOC message 3 process.
         */
	ret = edhoc_message_3_process(&resp_ctx, test_vector_1_message_3,
				      ARRAY_SIZE(test_vector_1_message_3));

	assert(EDHOC_SUCCESS == ret);
	assert(COMPLETED == resp_ctx.status);

	assert(EDHOC_TH_STATE_4 == resp_ctx.th_state);
	assert(ARRAY_SIZE(test_vector_1_th_4_raw) == resp_ctx.th_len);
	assert(0 ==
	       memcmp(test_vector_1_th_4_raw, resp_ctx.th, resp_ctx.th_len));

	assert(EDHOC_PRK_STATE_OUT == resp_ctx.prk_state);
	assert(ARRAY_SIZE(test_vector_1_prk_out) == resp_ctx.prk_len);
	assert(0 ==
	       memcmp(test_vector_1_prk_out, resp_ctx.prk, resp_ctx.prk_len));
...
void test_edhoc_x509_chain_edhoc_e2e(void)
{
	...
	assert(ARRAY_SIZE(test_vector_1_th_3_raw) == resp_ctx.th_len);
	assert(0 ==
	       memcmp(resp_ctx.th, test_vector_1_th_3_raw, resp_ctx.th_len));
	...
	assert(ARRAY_SIZE(test_vector_1_g_xy_raw) == init_ctx.dh_secret_len);
	assert(0 == memcmp(test_vector_1_g_xy_raw, init_ctx.dh_secret,
			   sizeof(init_ctx.dh_secret)));

	assert(EDHOC_TH_STATE_3 == init_ctx.th_state);
	assert(ARRAY_SIZE(test_vector_1_th_3_raw) == init_ctx.th_len);
	assert(0 ==
	       memcmp(test_vector_1_th_3_raw, init_ctx.th, init_ctx.th_len));
	...
	/**
         ...
         */
	uint8_t init_secret[ARRAY_SIZE(test_vector_1_oscore_secret_raw)] = { 0 };
	uint8_t init_salt[ARRAY_SIZE(test_vector_1_oscore_salt_raw)] = { 0 };
	...
	/**
         ...
         */
	uint8_t resp_secret[ARRAY_SIZE(test_vector_1_oscore_secret_raw)] = { 0 };
	uint8_t resp_salt[ARRAY_SIZE(test_vector_1_oscore_salt_raw)] = { 0 };
	...
	assert(0 == memcmp(test_vector_1_oscore_secret_raw, init_secret,
			   sizeof(test_vector_1_oscore_secret_raw)));
	assert(0 == memcmp(test_vector_1_oscore_secret_raw, resp_secret,
			   sizeof(test_vector_1_oscore_secret_raw)));
