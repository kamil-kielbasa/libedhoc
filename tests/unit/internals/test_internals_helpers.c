/**
 * \file    test_internals_helpers.c
 * \author  Kamil Kielbasa
 * \brief   Unit tests for edhoc_helpers.c buffer and CID utilities.
 *
...
TEST_TEAR_DOWN(internals_helpers)
{
	mbedtls_psa_crypto_free();
}

TEST(internals_helpers, set_connection_id_invalid_type)
{
...
TEST(internals_helpers, conn_id_equal_null)
{
...
TEST(internals_helpers, conn_id_equal_type_mismatch)
{
	struct edhoc_connection_id a = {
		.encode_type = EDHOC_CID_TYPE_ONE_BYTE_INTEGER,
		.int_value = 1,
...
TEST(internals_helpers, conn_id_equal_bstr_success)
{
	struct edhoc_connection_id a = {
		.encode_type = EDHOC_CID_TYPE_BYTE_STRING,
		.bstr_length = 2,
...
TEST(internals_helpers, prepend_flow_tiny_buf)
{
	struct edhoc_prepended_fields pf = {
		.buffer = NULL,
		.buffer_size = 0,
...
TEST(internals_helpers, extract_flow_info_forward)
{
...
TEST(internals_helpers, extract_conn_id_bstr)
{
...
TEST(internals_helpers, prepend_conn_id_bstr_tiny_buf)
{
	uint8_t buf[1];
	struct edhoc_prepended_fields pf = {
		.buffer = buf,
...
{
	RUN_TEST_CASE(internals_helpers, set_connection_id_invalid_type);
	RUN_TEST_CASE(internals_helpers, conn_id_equal_invalid_type);
	RUN_TEST_CASE(internals_helpers, conn_id_equal_null);
	RUN_TEST_CASE(internals_helpers, conn_id_equal_type_mismatch);
	RUN_TEST_CASE(internals_helpers, conn_id_equal_bstr_success);
	RUN_TEST_CASE(internals_helpers, prepend_conn_id_null);
	RUN_TEST_CASE(internals_helpers, prepend_conn_id_zero_buf);
	RUN_TEST_CASE(internals_helpers, prepend_conn_id_invalid_type);
	RUN_TEST_CASE(internals_helpers, prepend_conn_id_bstr_zero_len);
	RUN_TEST_CASE(internals_helpers, prepend_conn_id_bstr_success);
	RUN_TEST_CASE(internals_helpers, prepend_flow_null);
	RUN_TEST_CASE(internals_helpers, prepend_flow_success);
	RUN_TEST_CASE(internals_helpers, prepend_flow_tiny_buf);
	RUN_TEST_CASE(internals_helpers, prepend_recalculate_null);
	RUN_TEST_CASE(internals_helpers, prepend_recalculate_null_buf);
	RUN_TEST_CASE(internals_helpers, prepend_recalculate_null_msg_ptr);
	RUN_TEST_CASE(internals_helpers, extract_flow_info_null);
	RUN_TEST_CASE(internals_helpers, extract_flow_info_null_buf);
	RUN_TEST_CASE(internals_helpers, extract_flow_info_forward);
	RUN_TEST_CASE(internals_helpers, extract_conn_id_null);
	RUN_TEST_CASE(internals_helpers, extract_conn_id_int);
	RUN_TEST_CASE(internals_helpers, extract_conn_id_bstr);
	RUN_TEST_CASE(internals_helpers, prepend_conn_id_bstr_tiny_buf);
	RUN_TEST_CASE(internals_helpers, extract_conn_id_invalid_cbor);
	RUN_TEST_CASE(internals_helpers, extract_conn_id_null_buf);
}
