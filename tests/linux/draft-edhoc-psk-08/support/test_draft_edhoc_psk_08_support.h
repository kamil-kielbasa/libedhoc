/**
 * \file    test_draft_edhoc_psk_08_support.h
 * \author  Kamil Kielbasa
 * \brief   Shared helpers for the EDHOC-PSK known-answer (KAT) tests.
 *
 *          A derived key now lives only as a non-exportable key-store handle,
 *          so the draft's trace tests can no longer memcpy raw secrets into or
 *          out of the EDHOC context. These helpers bridge the gap:
 *            - import a fixed vector value as a key-store handle with the exact
 *              suite-2 attributes (ephemeral P-256 or HKDF derive key),
 *            - inject such a handle into an EDHOC key slot (isolated-step
 *              pre-condition),
 *            - verify an (ephemeral, peer-public) agreement equals the draft's
 *              shared secret,
 *            - verify a slot handle equals a draft vector via an HKDF-Expand
...
#ifndef TEST_DRAFT_EDHOC_PSK_08_SUPPORT_H
#define TEST_DRAFT_EDHOC_PSK_08_SUPPORT_H
 ...
 *        length compared by \ref tv_check_shared_secret (suite 2 is 32 bytes).
 */
#define TEST_DRAFT_EDHOC_PSK_08_OKM_LEN (32)
 ...
 * \param[in] g_xy                      Expected shared secret (draft vector).
					  ...
					  const uint8_t *g_xy, size_t g_xy_len)
{
	...
	TEST_ASSERT_TRUE(0 != g_xy_len &&
			 g_xy_len <= TEST_DRAFT_EDHOC_PSK_08_OKM_LEN);

	uint8_t raw[TEST_DRAFT_EDHOC_PSK_08_OKM_LEN] = { 0 };
	size_t raw_len = 0;
...
/**
 * \brief Assert that the key material in \p slot equals a draft vector.
 *
 ...
 * \param[in] ref                       Expected key material (draft vector).
	...
	enum edhoc_key_slot_id slot, const uint8_t *ref, size_t ref_len)
{
	...
	static const uint8_t info[] = { 'k', 'e', 'y', '-', 'c',
					'h', 'e', 'c', 'k' };
	uint8_t okm_ctx[TEST_DRAFT_EDHOC_PSK_08_OKM_LEN] = { 0 };
	uint8_t okm_ref[TEST_DRAFT_EDHOC_PSK_08_OKM_LEN] = { 0 };
...
#endif /* TEST_DRAFT_EDHOC_PSK_08_SUPPORT_H */
