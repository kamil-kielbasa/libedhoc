/**
 * \file    test_handshake_x5chain_dh_suite2.c
...
static int auth_cred_fetch_resp(void *user_ctx,
				struct edhoc_auth_credentials *auth_csred);
...
TEST_GROUP(handshake_x5chain_dh_suite2);

TEST_SETUP(handshake_x5chain_dh_suite2)
{
	ret = psa_crypto_init();
...
TEST_GROUP_RUNNER(handshake_x5chain_dh_suite2)
{
	RUN_TEST_CASE(handshake_x5chain_dh_suite2,
