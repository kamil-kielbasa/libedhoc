/**
 * \file    test_cipher_suite_24_x509_chain.c
 * \author  Kamil Kielbasa
 * \brief   Full EDHOC handshake integration test.
 *
 *          Scenario:
 *          - cipher suite 24 (P-384 / ES384 / A256GCM / SHA-384)
...
/* Signature identities (ECDSA over SHA-384, full uncompressed public key). */
static const struct hs_identity initiator_signature = {
	...
	.key_import = HS_KEY_ECDSA_SHA384,
	.public_key = PK_I,
...
/* Assemble the suite-24 scenario from the two endpoints and run the handshake. */
static void run_scenario(const char *name, enum edhoc_method method,
			 const struct handshake_endpoint *initiator,
			 const struct handshake_endpoint *responder)
{
...
TEST_GROUP(cipher_suite_24_x509_chain);

TEST_SETUP(cipher_suite_24_x509_chain)
{
	TEST_ASSERT_EQUAL(PSA_SUCCESS, psa_crypto_init());
}

TEST_TEAR_DOWN(cipher_suite_24_x509_chain)
{
	mbedtls_psa_crypto_free();
}

TEST(cipher_suite_24_x509_chain, method_0)
{
	const uint8_t init_cid_value[] = { 0x37 };
	const struct edhoc_buffer init_cid = {
		.value = init_cid_value,
		.length = ARRAY_SIZE(init_cid_value),
	};
	const uint8_t resp_cid_value[] = { 0x18, 0x19, 0x1a };
	...
	run_scenario("cipher suite 24, X.509 chain, method 0", EDHOC_METHOD_0,
		     &initiator, &responder);
}

TEST_GROUP_RUNNER(cipher_suite_24_x509_chain)
{
	RUN_TEST_CASE(cipher_suite_24_x509_chain, method_0);
}
