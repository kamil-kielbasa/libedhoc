/**
 * \file    test_cipher_suite_0_x509_chain.c
 * \author  Kamil Kielbasa
 * \brief   Full EDHOC handshake integration test.
 *
 *          Scenario:
 *          - cipher suite 0 (X25519 / EdDSA / AES-CCM-16-64-128 / SHA-256)
...
/* Signature identities presenting a single leaf certificate. */
...
/* The same identities presenting a two-certificate chain (leaf then CA), each
 * expecting the mirrored order from the peer. */
...
/* Assemble the suite-0 scenario from the two endpoints and run the handshake. */
static void run_scenario(const char *name, enum edhoc_method method,
			 const struct handshake_endpoint *initiator,
			 const struct handshake_endpoint *responder)
{
...
TEST_GROUP(cipher_suite_0_x509_chain);

TEST_SETUP(cipher_suite_0_x509_chain)
{
	TEST_ASSERT_EQUAL(PSA_SUCCESS, psa_crypto_init());
}

TEST_TEAR_DOWN(cipher_suite_0_x509_chain)
{
	mbedtls_psa_crypto_free();
}

TEST(cipher_suite_0_x509_chain, method_0_one_certificate)
{
	...
	run_scenario("cipher suite 0, X.509 chain, method 0, one certificate",
		     EDHOC_METHOD_0, &initiator, &responder);
}

TEST(cipher_suite_0_x509_chain, method_0_two_certificate_chain)
{
	const struct edhoc_connection_id init_cid = {
		.encode_type = EDHOC_CONNECTION_ID_TYPE_ONE_BYTE_INTEGER,
		.int_value = -1,
	...
	run_scenario("cipher suite 0, X.509 chain, method 0, two certificates",
		     EDHOC_METHOD_0, &initiator, &responder);
}

TEST_GROUP_RUNNER(cipher_suite_0_x509_chain)
{
	RUN_TEST_CASE(cipher_suite_0_x509_chain, method_0_one_certificate);
	RUN_TEST_CASE(cipher_suite_0_x509_chain,
