/**
 * \file    test_mem_custom_group.c
 * \author  Kamil Kielbasa
 * \brief   Out-of-memory sweep for the custom EDHOC memory backend: an
 *          exhaustive fault-injection test proving that every allocation
 *          failure path releases all previously allocated buffers.
 *
...
TEST(mem_custom_handshake, every_allocation_failure_frees_all_priors)
{
	...
	/*
	 * Baseline: run one full handshake to learn how many allocations it
	 * makes and to confirm the success path is allocation-balanced.
	 */
	ret = test_mem_custom_setup_contexts(&initiator, &responder);
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);

	edhoc_mem_reset();

	ret = test_mem_custom_drive_handshake(&initiator, &responder);
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);
	...
	ret = edhoc_context_deinit(&initiator);
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);

	ret = edhoc_context_deinit(&responder);
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);

	/*
	 * Sweep: arm a fault at the nth allocation for every n in [1, count]
	 * and confirm the library unwinds cleanly, leaving nothing live.
	 */
	...
	for (size_t nth = 1; nth <= alloc_count; ++nth) {
		...
		ret = test_mem_custom_setup_contexts(&init_oom, &resp_oom);
		TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);

		edhoc_mem_reset();
		edhoc_mem_fail_on_alloc(nth);

		ret = test_mem_custom_drive_handshake(&init_oom, &resp_oom);
		...
		ret = edhoc_context_deinit(&init_oom);
		TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);

		ret = edhoc_context_deinit(&resp_oom);
		TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);
	}
