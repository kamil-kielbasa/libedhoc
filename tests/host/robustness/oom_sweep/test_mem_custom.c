...
/**
 * \brief Instrumented allocator state: live counters plus the armed fault.
 */
struct mem_state {
...
void *edhoc_mem_alloc(size_t size)
{
	 ...
	 */
	struct block_header *const hdr = calloc(1, sizeof(*hdr) + size);
