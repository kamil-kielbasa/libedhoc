/**
 * \file    benchmark_credentials.h
 * \author  Kamil Kielbasa
 * \brief   Authentication-credential callbacks for the benchmark harness.
 *
 *          One \ref benchmark_identity describes a single EDHOC peer. A
 *          benchmark case declares the Initiator's and Responder's identity
 *          once; each endpoint then points \c own at the identity it presents
 *          and \c peer at the identity it must verify (see \ref
 *          benchmark_endpoint in benchmark_suite.h):
 *
...
struct benchmark_identity {
	...
	enum benchmark_key_import key_import;
	...
	struct benchmark_cert
		cert[CONFIG_LIBEDHOC_MAX_NR_OF_CERTS_IN_X509_CHAIN];
