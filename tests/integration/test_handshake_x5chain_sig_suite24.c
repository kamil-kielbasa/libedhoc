/**
 * \file    test_handshake_x5chain_sig_suite24.c
...
/* Test vector header: */
#include "test_vector_x5chain_sign_keys_suite_24.h"
...
#define DH_KEY_AGREEMENT_LENGTH (48)
					    ...
					    struct edhoc_auth_creds *auth_cred)
{
	...
	const int res = edhoc_cipher_suite_24_key_import(
		NULL, EDHOC_KT_SIGNATURE, SK_I, ARRAY_SIZE(SK_I),
					    ...
					    struct edhoc_auth_creds *auth_cred)
{
	...
	const int res = edhoc_cipher_suite_24_key_import(
		NULL, EDHOC_KT_SIGNATURE, SK_R, ARRAY_SIZE(SK_R),
		auth_cred->priv_key_id);
...
TEST_GROUP(handshake_x5chain_sig_suite24);

TEST_SETUP(handshake_x5chain_sig_suite24)
{
	ret = psa_crypto_init();
	...
	const struct edhoc_cipher_suite cipher_suites[] = {
		*edhoc_cipher_suite_24_get_suite(),
	};
	...
	ret = edhoc_bind_keys(init_ctx, edhoc_cipher_suite_24_get_keys());
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);

	ret = edhoc_bind_crypto(init_ctx, edhoc_cipher_suite_24_get_crypto());
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);
	...
	ret = edhoc_bind_keys(resp_ctx, edhoc_cipher_suite_24_get_keys());
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);

	ret = edhoc_bind_crypto(resp_ctx, edhoc_cipher_suite_24_get_crypto());
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);
...
TEST_TEAR_DOWN(handshake_x5chain_sig_suite24)
{
 ...
 * @env       PSA crypto init, cipher suite 24, method from test vector, X.509 chain credentials (signature keys).
 ...
 */
TEST(handshake_x5chain_sig_suite24, one_cert_in_chain)
{
...
TEST_GROUP_RUNNER(handshake_x5chain_sig_suite24)
{
	RUN_TEST_CASE(handshake_x5chain_sig_suite24, one_cert_in_chain);
}
