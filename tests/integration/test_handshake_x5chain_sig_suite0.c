/**
 * \file    test_handshake_x5chain_sig_suite0.c
...
/* Import a 64-byte Ed25519 private key (Compact25519 seed||pub) as an
 * exportable RAW_DATA key: mbedTLS/PSA has no software EdDSA, so the suite
 * exports it and signs with Compact25519. */
...
TEST_GROUP(handshake_x5chain_sig_suite0);

TEST_SETUP(handshake_x5chain_sig_suite0)
{
	ret = psa_crypto_init();
	...
	ret = edhoc_bind_crypto(init_ctx, edhoc_cipher_suite_0_get_crypto());
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);
	...
	ret = edhoc_bind_crypto(resp_ctx, edhoc_cipher_suite_0_get_crypto());
	TEST_ASSERT_EQUAL(EDHOC_SUCCESS, ret);
...
TEST_TEAR_DOWN(handshake_x5chain_sig_suite0)
{
...
TEST(handshake_x5chain_sig_suite0, one_cert_in_chain)
{
...
TEST(handshake_x5chain_sig_suite0, two_certs_in_chain)
{
...
TEST_GROUP_RUNNER(handshake_x5chain_sig_suite0)
{
	RUN_TEST_CASE(handshake_x5chain_sig_suite0, one_cert_in_chain);
	RUN_TEST_CASE(handshake_x5chain_sig_suite0, two_certs_in_chain);
}
